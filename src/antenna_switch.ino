...
const int pin_relay_hexbeam = 5;
const int pin_relay_aperiodic = 6;
const int button_antennas_mode = 9;
const int bands_input = A0;
const int power_input = A1;
...
void setup() {
  ...
  pinMode(button_antennas_mode, INPUT_PULLUP);
...
void loop() {
  int raw = read_analog_io (bands_input);  
    ...
    if (active_rig == power_on && button_pressed(button_antennas_mode) == LOW) {
